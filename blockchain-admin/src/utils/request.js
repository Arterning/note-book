...
service.interceptors.request.use(
  config => {
    ...
    signParams(testData)
    config.headers.sign = testData.sign
    config.headers.timestamp = testData.timestamp
    config.headers.sessionId = testData.sessionId
...
function handleParams(config) {
  ...
  const { data, isUpload, needAppType, noLogin } = config
  ...
  if (isUpload) {
    ...
    needAppType ? (data['appType'] = '2') : null
  }
}
