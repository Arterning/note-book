<template>
  ...
  <view class="city-select">
    ...
    >
      <!-- 预留搜索-->
      <view v-if="isSearch" class="city-serach"
        ><input
      ...
      /></view>
      <!-- 当前定位城市 -->
      <!-- <view v-if="activeCity && !serachCity" class="hot-title"
        >当前定位城市</view
      >
      <view v-if="activeCity && !serachCity" class="hot-city">
        <view class="hot-item" @click="cityTrigger(activeCity)">{{
          activeCity[formatName]
        }}</view>
      </view> -->
      <!-- 热门城市 -->
      <view v-if="hotCity.length > 0 && !serachCity" class="hot-title"
        >热门城市</view
      >
      <view v-if="hotCity.length > 0 && !serachCity" class="hot-city">
      ...
      <!-- 城市列表(搜索前) -->
      <view v-if="!serachCity" class="citys">
        <view
      ...
      <view v-if="serachCity" class="citys">
        <view
          ...
          class="citys-row"
        >
          <view :key="inx" class="citys-item" @click="cityTrigger(item)">{{
            item.name
          }}</view>
        </view>
      </view>
    ...
    <!-- 城市选择索引-->
    <view v-if="!serachCity" class="city-indexs-view">
...
export default {
  ...
  methods: {
    ...
    buildCityindexs() {
      ...
      for (let i = 0; i < this.cityData.length; i++) {
        // 获取首字母
        const cityLetter = this.getLetter(this.cityData[i][this.formatName])
          .firstletter
        // 获取当前城市首字母的unicode，用作后续排序
        const unicode = this.getLetter(this.cityData[i][this.formatName])
          .unicode
