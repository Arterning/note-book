 ...
 * @LastEditTime : 2022-08-24 10:18:14
 * @FilePath     : \blockchain-admin\src\views\login\components\SetPasswordForm.vue
-->
<template>
    ...
    async submit() {
      if (this.form.password1 === this.form.password2) {
        this.$refs.form.validate(valid => {
          if (valid) {
            // 调用密码接口，成功后返回登录
            this.$emit('submit-loading', true)

            this.$store
              .dispatch('user/setPassword', this.form)
              .then(() => {
                this.$emit('submit-loading', false)
