 ...
 * @LastEditTime : 2022-09-02 16:20:57
 * @FilePath     : \blockchain-admin\src\views\login\components\LoginForm.vue
-->
<template>
  <el-form
    ref="form"
      ...
      if (validPhoneNumber(username)) {
        try {
          ...
          if (!valid) {
            ...
            // if (needSetPassword === 'Y') {
            //   if (
            //     !Object.prototype.hasOwnProperty.call(this.hasAlerted, username)
            //   ) {
            //     this.hasAlerted[username] = false
            //   }
            //   if (!this.hasAlerted[username]) {
            //     this.$alert(
            //       '您可使用短信验证码方式登录系统，请在本次登录成功后设置密码。',
            //       '提示',
            //       {
            ...
            //     )
            //       .then(() => {
            //         this.checkUsernameCallback(username)
            //       })
            //       .catch(() => {
            //         this.checkUsernameCallback(username)
            //       })
            ...
            //   }
            // } else {
            //   this.isPasswordLoginDisabled = false
            // }
          }
        } catch (error) {
