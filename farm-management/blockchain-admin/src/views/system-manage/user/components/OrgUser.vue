 ...
 * @LastEditTime : 2022-09-13 13:55:57
...
export default {
  ...
  methods: {
    ...
    async getRoleInfo() {
      try {
        const { roleInfoList } = await getRole({ appType: '2' })
    ...
    onSubmitUserDialog() {
      ...
      if (this.dialogFlag === 'add') {
        ...
        this.handleSubmitUserDialog(api, payload, '新建')
        ...
        this.handleSubmitUserDialog(api, payload, '修改')
      }
    },
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          try {
            ...
            this.$message({
              message: `${this.dialogFlag === 'add' ? '新建' : '修改'}成功`,
              type: 'success',
              duration: 2000,
              onClose: () => {
