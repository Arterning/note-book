 ...
 * @LastEditTime : 2022-09-13 13:56:06
 * @FilePath     : \blockchain-admin\src\views\system-manage\user\index.vue
-->
<template>
  <div class="user">
    <i-tab>
      <i-action @on-query="getTableData" @on-reset="onReset">
        ...
        <template #action>
          <el-button type="primary" @click="clickAdd()" v-btn-permission:1721
            >新建</el-button
          >
        </template>
      </i-action>
...
export default {
  ...
  methods: {
    // 获取角色列表
    async getRole() {
      const { data } = await getRole({ appType: '2', enable: 'Y' })
      this.roleList = data
    },
    ...
    onSubmitStaffialog() {
      ...
      if (this.dialogFlag === 'add') {
        console.log('add')
        api = createStaff
        payload = { ...this.ruleForm, appType: '2' }
