...
func createRouter() *gin.Engine {
	...
	route := router.Group("/")
	{
		...
		route.POST("putUploadRecord", handler.PutUploadRecord)               // 进粮清单ID与种植ID数据上链
		route.POST("putDict", handler.PutDict)                               // 农事字典数据上链
		route.POST("putPesticideFertilizer", handler.PutPesticideFertilizer) // 农药、化肥字典数据上链

		route.GET("/queryUploadRecords/:uuid", handler.QueryUploadRecords) // 第三方上传历史数据溯源
		route.GET("/queryUploadRecord/:uuid", handler.QueryUploadRecord)   // 第三方上传数据溯源
		...
		//route.GET("/queryFeedSectionMap/:uuid", handler.QueryFeedSectionMap) // 进粮清单ID与种植ID关联关系查询
