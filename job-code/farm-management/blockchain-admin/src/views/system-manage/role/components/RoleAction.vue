 ...
 * @LastEditTime : 2022-09-23 14:13:19
 * @FilePath     : \blockchain-admin\src\views\system-manage\role\components\RoleAction.vue
-->
<template>
...
<script>
// import { getRoleRight, getRoleUser, setRoleRight,submitRoleRight } from '@/api/system-manage'
...
export default {
  ...
  data() {
    return {
      ...
      visible: false,
      codeObj: {
        dataPermissionProvince: '320000',
        dataPermissionCity: '320100',
        dataPermissionArea: '320111',
        dataPermissionAddress: '湖南省长沙市xxxx县'
      },
      //默认勾选数组
  ...
  methods: {
    ...
    /**
     * @description: 权限设置 - 数据权限
     * @param {String} event 0 所在组织及下级；1 全组织
     * @author: Hemingway
     */
    // async onRadioChange(event) {
    //   this.$parent.role.dataPermissionType = await this.radioChangePromise(
    //     event
    //   )
    // },

    /**
     * @description: 数据权限交互promise
     * @param {String} event
     * @author: Hemingway
     */
    // radioChangePromise(event) {
    //   return new Promise(resolve => {
    //     let tmp = ''

    //     if (event === '1') {
    //       this.$confirm(
    //         `此操作将为角色：${this.role.name}，开放组织下全部实体农场的访问及管理权限，是否继续？`,
    //         '提示',
    //         {
    ...
    async getRoleRight(role) {
      try {
        ...
        let echoArr = []
        // this.codeObj.dataPermissionAddress = data.dataPermissionAddress ? data.dataPermissionAddress : `后台返显字段值:${data.dataPermissionAddress}`
        ...
        if (Array.isArray(data.menuTree) && data.menuTree.length > 0) {
          ...
          console.log('树形会笑0', data.menuTree)
